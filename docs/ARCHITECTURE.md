...
- `ghcr.rs` — GitHub packages API listing (`/user/packages`, `/users/<owner>/packages`), used instead of `/v2/_catalog` for `RegistryType::Ghcr`, plus `list_owners` (`/user`, `/user/orgs`) feeding the owner picker's suggestions — infallible, since those suggestions are a convenience and `/user/orgs` needs a scope GHCR browsing doesn't. A free function rather than a `RegistryClient` method because `api.github.com` is a different origin from `ghcr.io` — see `docs/ghcr-registry-browsing.md`.
- `ecr.rs` — AWS ECR discovery and authorization via the AWS SDK, used instead of `/v2/_catalog` for `RegistryType::Ecr` and `EcrPublic`: `connect`/`connect_public` (one `GetAuthorizationToken` + paginated `DescribeRepositories`, returning the endpoint, the minted token and the repository list together, since the client cannot be built without the first two) and `authorize`/`authorize_public` for the credential's own refresh. Free functions for the same reason as `ghcr.rs` — `api.ecr.<region>.amazonaws.com` is a different origin from the registry. Also holds the pure, unit-tested helpers: `parse_aws_profiles` (both AWS config file shapes), `AWS_REGIONS`, and `describe_failure`, which replaces the SDK's six-provider chain error with an actionable one-liner. See `docs/ecr-registry-browsing.md`.
- `pagination.rs` — RFC 5988 `Link` header parser for paginated catalog/tag responses.

## tui/

- `app.rs` — All mutable UI state: `Focus` (Repos/Tags/Detail), `LoadState`, `SortOrder`, modal structs (`InspectModal`, `LayerDiffModal`), status message with TTL. `Modal::ArtifactoryPicker` is the repo-key picker shown after switching to an Artifactory profile (one-shot fetch, filtered locally — unlike `SearchPicker`'s incremental server-side Docker Hub search). `Modal::GhcrPicker` is the same shape for GHCR packages and shares its renderer (`ui.rs::draw_filter_picker_modal`), but its one fetch also populates the Repos pane: a GHCR package is a repository, not a sub-registry, so there is no second fetch to fill the pane with. `Modal::GhcrOwnerPicker` sits one level above it (`Backspace`/`u`), combining a text box with fetched suggestions — its rows come from the pure `picker_choices`, shared with the renderer so the row opened is the row highlighted. `Modal::EcrProfilePicker` and `Modal::EcrRegionPicker` are the same shape for ECR's two switchable axes (AWS profile → region), reusing `picker_choices` and sharing `HelpContext::ChoicePicker` with the owner picker; neither is applied until both stages are confirmed, and `ecr_repo_cache` is keyed `(profile, aws_profile, region)` so a switch cannot serve the previous account's repositories. `Modal::Help` carries a `HelpContext`, set from the pure `help_context_for(modal, focus)` when `?` is pressed (see `docs/keybindings.md`); `App::help_return` stashes whatever modal `?` was pressed over (including `Modal::None` in normal mode) so closing Help restores it exactly, filter text and selection included.
