...
- **Two realm guards, on purpose.** `is_trusted_realm` (Basic/Bearer) allows a realm sharing the last two DNS labels, because Docker Hub splits `auth.docker.io` from `registry-1.docker.io`. `is_same_origin_realm` (access token only) requires an exact host *and* port. Don't merge them: an access token is a platform-wide JFrog credential, and the looser rule would trust every host under the registry's registered domain.
